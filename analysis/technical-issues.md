...
## 9. [RESOLVED] Predictable step-up elevation tokens (`mfa-orchestrator`)

- **Status:** FIXED (2026-08-30)
- **Details:** `packages/modules/auth/src/modules/mfa-orchestrator/services/mfa.service.ts` generated the step-up `elevationToken` client-side from `Date.now()` (`elevated-totp-${Date.now()}` / `elevated-passkey-${Date.now()}`) and persisted it to `sessionStorage` via `useStepUpAuth`, i.e. handled it as a bearer credential. `stepUp.verifyTotp` additionally hard-coded `success: true`, ignoring the server verdict. No code currently transmits the token to the backend, so exploitability was low, but the value was credential-shaped and predictable.
- **Resolution:** `verifyBiometric` / `verifyTotp` now prefer the server-issued token (`res.data.token` / `res.data.elevationToken`) and honour `res.data.expiresAt`; the client-side fallback is `crypto.randomUUID()` with a `crypto.getRandomValues` path and throws if no CSPRNG is available (fail closed). `verifyTotp` derives `success` from the response body. `useStepUpAuth.handleVerificationSuccess` now rejects any result without `success && elevationToken`. Correlation-only ids (`WorkerApiClientProxy` `requestId`, `api.client.ts` `X-Request-ID`) were also moved to guarded `crypto.randomUUID()` for consistency — not a security fix. Hook tests: `useStepUpAuth.test.ts` 5/5 pass; `tsc --noEmit` clean for the changed files.
- **Backend companion fix:** `Authentication/app/services/sync_service.ts` (directory pull-sync) was assigning synced users a `Math.random()`-derived password on every run via `updateOrCreate`. Now uses `node:crypto` `randomBytes(32)` and `firstOrNew(...)` so the password is set only on first provisioning (`!user.$isPersisted`) and never regenerated on subsequent syncs. See `Authentication/docs/context/components/identity-protocols.md` and `.jules/sentinel.md` (2026-08-30).
