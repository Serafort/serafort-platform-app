...
### 2.1 Phantom packages referenced in _live_ configuration
...
| `packages/layout`        | _(none declared)_      | `typescript: "^7.0.2"` |

Three different major/minor ranges are in play depending on which package's `devDependencies` pnpm happens to hoist/resolve at install time. This is exactly the class of "stale version reference" drift already being tracked and corrected in the in-flight documentation overhaul (per prior session notes) — this confirms the drift is real in the _package manifests themselves_, not only in prose docs. **Recommend:** pin one TypeScript range at the workspace root and remove the per-package overrides unless a specific package has a proven reason to diverge.
