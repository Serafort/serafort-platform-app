...
| Plain `{ path, element: <GuestRoute element={...}/>, layout: 'noLayout' }` | Relies on `assembleApp`'s `LayoutRouteWrapper` reading the _outer_ config's `layout` field | ✅ Yes — `assembleApp` wraps every route, so `'noLayout'` (and `'none'`/`'admin'` handling) is applied for bare elements too |
