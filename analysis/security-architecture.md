...
| `mfa-orchestrator/services/mfa.service.ts`<br>`mfa-orchestrator/hooks/useStepUpAuth.ts` | **V2.8** Step-Up / Re-Authentication<br>**V6.3** Random Values | **CWE-330** (Insufficiently Random Values)<br>**CWE-603** (Client-Side Enforcement of Server-Side Security) | • Elevation/step-up tokens must be server-issued; client fallback uses a CSPRNG (`crypto.randomUUID` / `getRandomValues`) and fails closed — never `Math.random()` / `Date.now()`.<br>• `success` must be derived from the server response, not hard-coded.<br>• Client "isElevated" state is UX only; the backend must re-verify elevation on every sensitive call.<br>• **Status: RESOLVED 2026-08-30** — see `technical-issues.md` §9 and `.jules/sentinel.md`. |
