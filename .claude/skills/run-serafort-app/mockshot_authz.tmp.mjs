...
for (let i = 0; i < 2; i++) {
  ...
  if (await page.locator('text=Access Forbidden').count()) { await page.reload({ waitUntil: 'domcontentloaded' }); await page.waitForLoadState('networkidle', { timeout: 30000 }).catch(() => {}) }
}
await page.waitForSelector('main h1, h1, [data-testid=screen-ready]', { timeout: 120000 }).catch(() => {})
