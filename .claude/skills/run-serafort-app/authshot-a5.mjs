...
const emailInput = page.locator('input:not([type=password]):not([type=checkbox]):visible').first()
await emailInput.click(); await emailInput.pressSequentially(EMAIL)
const pw = page.locator('input[type=password]').first()
await pw.click(); await pw.pressSequentially(PASSWORD)
...
for (const route of routes) {
  ...
  await page.waitForTimeout(3000); await page.waitForFunction(() => !document.querySelector('[role=progressbar]') || document.querySelectorAll('[role=progressbar]').length===0, null, {timeout: 20000}).catch(()=>{}); await page.waitForTimeout(1200)
