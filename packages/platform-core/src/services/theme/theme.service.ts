...
export const themeService = {
  ...
  getTheme: async (orgId: string, currentVersion?: string): Promise<TenantThemeConfig | null> => {
    ...
    const requestPromise = (async () => {
      try {
        ...
        const orgQuery =
          orgId && orgId !== 'default' && orgId !== 'current' ? `?orgId=${orgId}` : ''
        try {
          const response = await apiClient.get<any>(
            `/api/v1/themes/tenant${orgQuery}`,
            requestOptions,
          )
          ...
          const response = await apiClient.get<TenantThemeConfig>(
            `/api/admin/organizations/${orgId}/styles`,
            requestOptions,
          )
          if (response.status === 304) return null
  ...
  saveTheme: async (theme: TenantThemeConfig): Promise<void> => {
    const orgQuery =
      ...
      theme.organizationId !== 'default' &&
      theme.organizationId !== 'current'
    ...
    try {
      ...
      const response = await apiClient.post(
        `/api/admin/organizations/${theme.organizationId || 'default'}/styles`,
        theme,
      )
