...
export const THEME_QUERY_KEYS = {
  all: ['theme'] as const,
  tenant: (organizationId: string | number = 'current') => ['theme', 'tenant', String(organizationId)] as const,
...
export function useTenantTheme(
  ...
  options?: Omit<UseQueryOptions<FetchResponse<TenantThemeResponse>, HttpError>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
...
export function useUpdateTenantTheme(
  ...
  return useMutation({
    ...
    onMutate: async (newThemePayload) => {
      ...
      const previousTheme = queryClient.getQueryData<FetchResponse<TenantThemeResponse>>(queryKey)
      ...
      if (previousTheme) {
        queryClient.setQueryData<FetchResponse<TenantThemeResponse>>(queryKey, {
          ...
          data: {
            ...
            presetId: newThemePayload.presetId !== undefined ? newThemePayload.presetId : previousTheme.data.presetId,
            isDark: newThemePayload.isDark !== undefined ? newThemePayload.isDark : previousTheme.data.isDark,
            isCustom: true,
          }
        })
      }
...
export function useGenerateAiTheme(
  organizationId?: string | number,
  options?: UseMutationOptions<FetchResponse<AiThemeResponse>, HttpError, GenerateAiThemeRequest>
) {
  return useMutation({
    mutationFn: (payload: GenerateAiThemeRequest) => themeService.generateAiTheme(payload, organizationId),
    ...options,
  })
}
...
export function useThemePresets(
  options?: Omit<UseQueryOptions<FetchResponse<ThemePresetsResponse>, HttpError>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
