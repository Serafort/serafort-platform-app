...
export interface GenerateAiThemeRequest {
  ...
  isDark?: boolean
  useLlm?: boolean
  providerType?: 'gemini' | 'openai' | 'anthropic' | 'mistral' | 'groq' | 'cohere' | 'openrouter' | string
  model?: string
  apiKey?: string
}
...
export class ThemeService {
  private static getHeaders(organizationId?: string | number): Record<string, string> {
   ...
   */
  static async getTenantTheme(organizationId?: string | number): Promise<FetchResponse<TenantThemeResponse>> {
    const headers = this.getHeaders(organizationId)
    const query = organizationId && organizationId !== 'default' ? `?orgId=${organizationId}` : ''
  ...
  static async saveTenantTheme(
    ...
    const headers = this.getHeaders(organizationId)
    const query = organizationId && organizationId !== 'default' ? `?orgId=${organizationId}` : ''
    return apiClient.put<TenantThemeResponse>(`/api/v1/themes/tenant${query}`, payload, {
      headers,
    })
  }
