...
export const CURATED_PROMPT_SUGGESTIONS: PromptSuggestion[] = [
  ...
  },
  {
    ...
    category: "Clean SaaS",
    prompt:
      "Clean modern fintech SaaS with deep sapphire blue primary, emerald green success accents, crisp white surfaces and subtle shadows",
  ...
  },
  {
    ...
    prompt:
      "Ultra-luxurious obsidian dark theme with champagne gold primary, warm bronze secondary, deep onyx glass panels and refined golden borders",
  ...
  },
  {
    ...
    prompt:
      "Warm organic aesthetic with roasted terracotta primary, sage green secondary, warm cream background and soft rounded pill corners",
  ...
  },
  {
    ...
    prompt:
      "Retro neo-brutalist theme with electric canary yellow primary, hot coral secondary, sharp 0px corners, and bold 3px black offset shadows",
  ...
  },
  {
    ...
    category: "Modern Dark",
    prompt:
      "Translucent frosted glass bento theme with aurora violet primary, electric indigo secondary, and deep space glass background with 24px blur",
...
export function getWcagComplianceBadge(
  foregroundHex: string,
  backgroundHex: string,
): { label: string; color: "success" | "warning" | "error"; ratio: number } {
  const ratio = getContrastRatio(foregroundHex, backgroundHex);
  if (ratio >= 7)
    return { label: `AAA (${ratio.toFixed(1)}:1)`, color: "success", ratio };
  if (ratio >= 4.5)
    return { label: `AA (${ratio.toFixed(1)}:1)`, color: "success", ratio };
  if (ratio >= 3)
    return {
      label: `AA Large (${ratio.toFixed(1)}:1)`,
      color: "warning",
      ratio,
    };
...
class AiThemePromptService {
  ...
  analyzePrompt(prompt: string): PromptAnalysisResult {
    ...
    if (
      ...
      text.includes("neu") ||
      text.includes("soft ui") ||
      text.includes("molded") ||
    ...
    if (presetMatch === "cyberpunk-hud") {
      ...
      presetMatch === "pure-brutalism" ||
      presetMatch === "neo-brutalism"
    ) {
  ...
  generateThemeFromPrompt(
    ...
    const base =
      baseConfig || applyPreset(analysis.presetMatch) || DEFAULT_TENANT_THEME;
    ...
    const newTheme: TenantThemeConfig = {
      ...
      effects: normalizeEffectConfig({
        ...
        glassmorphism: {
          ...
          blur:
            analysis.effectType === "glass"
              ? "20px"
              : base.effects.glassmorphism.blur,
          background: analysis.isDark
            ? "rgba(15, 23, 42, 0.65)"
            : "rgba(255, 255, 255, 0.65)",
          borderColor: analysis.isDark
        ...
        return {
          ...
          tokens: {
            ...
            colors: {
              ...
              primary: {
                value:
                  serverConfig.tokens?.colors?.primary?.value ||
                  serverConfig.tokens?.colors?.primary ||
                  "#6366f1",
                description: "AI Generated Primary",
              } as ColorToken,
              secondary: {
                value:
                  serverConfig.tokens?.colors?.secondary?.value ||
                  serverConfig.tokens?.colors?.secondary ||
                  "#8b5cf6",
                description: "AI Generated Secondary",
              } as ColorToken,
              background: {
                value:
                  serverConfig.tokens?.colors?.background?.value ||
                  serverConfig.tokens?.colors?.background ||
                  "#09090b",
                description: "AI Generated Background",
              } as ColorToken,
              surface: {
                value:
                  serverConfig.tokens?.colors?.surface?.value ||
                  serverConfig.tokens?.colors?.surface ||
                  "#18181b",
                description: "AI Generated Surface",
              } as ColorToken,
              text: {
                value:
                  serverConfig.tokens?.colors?.text?.value ||
                  serverConfig.tokens?.colors?.text ||
                  "#fafafa",
                description: "AI Generated Text",
              } as ColorToken,
              textMuted: {
                value:
                  serverConfig.tokens?.colors?.textMuted?.value ||
                  serverConfig.tokens?.colors?.textMuted ||
                  "#71717a",
                description: "AI Generated Muted Text",
              } as ColorToken,
              border: {
                value:
                  serverConfig.tokens?.colors?.border?.value ||
                  serverConfig.tokens?.colors?.border ||
                  "#27272a",
                description: "AI Generated Border",
              } as ColorToken,
            },
