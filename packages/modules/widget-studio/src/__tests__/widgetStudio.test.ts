...
describe("Sanitizer Layer", () => {
  it("sanitizes prompt and strips HTML tags and code blocks", () => {
    const raw =
      "Create a sales chart with ```code``` and <script>alert(1)</script>";
  ...
  it("extracts valid JSON from markdown code fence", () => {
    const markdown =
      'Here is your widget definition:\n```json\n{"id": "w1", "name": "Test Widget"}\n```';
...
describe("Validation Agent", () => {
  ...
  it("catches invalid SemVer and invalid dimensions", () => {
    ...
    expect(result.schemaErrors.some((e: string) => e.includes("SemVer"))).toBe(
      true,
    );
    expect(result.schemaErrors.some((e: string) => e.includes("width"))).toBe(
      true,
    );
    expect(result.schemaErrors.some((e: string) => e.includes("height"))).toBe(
      true,
    );
  });
});
...
describe("Query Keys & API Configuration", () => {
  it("produces consistent query keys", () => {
    ...
    expect(WIDGET_STUDIO_KEYS.dashboardLayouts()).toEqual([
      "dashboard",
      "layouts",
    ]);
  });
