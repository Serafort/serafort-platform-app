...
export function validateWidgetDsl(
  ...
  if (dsl.layout) {
    ...
    if (![200, 280, 340, 400].includes(height)) {
      schemaErrors.push(`layout.height must be 200, 280, 340, or 400 — got ${height}`)
    }
  }
  ...
  if (dsl.props) {
    const checkValue = (val: unknown, path: string) => {
      ...
      } else if (typeof val === 'string' && DANGEROUS_PATTERNS.some((p) => p.test(val))) {
        securityErrors.push(`Dangerous string value at props.${path}: "${val.slice(0, 50)}"`)
