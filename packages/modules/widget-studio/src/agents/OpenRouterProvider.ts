/**
 * OpenRouterProvider — DISABLED client-side provider.
 *
 * SECURITY: Browser-side AI provider calls required an API key to be shipped in
 * the public JS bundle (`VITE_OPENROUTER_API_KEY`) and sent from the user's
 * browser with an `Authorization: Bearer` header. That is a credential-exposure /
...
const DISABLED_MESSAGE =
  'Client-side OpenRouterProvider is disabled. AI generation runs server-side via ' +
...
export class OpenRouterProvider implements AIProvider {
  ...
  async generate(options: AIGenerateOptions): Promise<AIGenerateResult> {
    console.error('[OpenRouterProvider]', DISABLED_MESSAGE)
