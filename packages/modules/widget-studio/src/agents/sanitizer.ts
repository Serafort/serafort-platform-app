...
export function extractJson(text: string): unknown | null {
  ...
  const cStart =
    oStart === -1 ? aStart : aStart === -1 ? oStart : Math.min(oStart, aStart);
 ...
 */
export function sanitizeWidgetDsl(
  rawDsl: unknown,
): import("@cap/shared-types").WidgetDefinition {
  const dsl = (rawDsl && typeof rawDsl === "object" ? rawDsl : {}) as Record<
    string,
    unknown
  >;

  const rawWidth =
    typeof dsl.layout === "object" && dsl.layout !== null
      ? (dsl.layout as any).width
      : undefined;
  const rawHeight =
    typeof dsl.layout === "object" && dsl.layout !== null
      ? (dsl.layout as any).height
      : undefined;

  const width = [4, 8, 12].includes(rawWidth) ? (rawWidth as 4 | 8 | 12) : 8;
  const height = [200, 280, 340, 400].includes(rawHeight)
    ? (rawHeight as 200 | 280 | 340 | 400)
    : 280;
  ...
  const version =
    typeof dsl.version === "string" && /^\d+\.\d+\.\d+$/.test(dsl.version)
      ? dsl.version
      : "1.0.0";
  const component =
    typeof dsl.component === "string" && dsl.component.trim()
      ? dsl.component.trim()
      : "core-dynamic-layout";

  const rawProps = (
    dsl.props && typeof dsl.props === "object" ? dsl.props : {}
  ) as Record<string, unknown>;
  const props = sanitizeObject(rawProps);

  const rawBehavior = (
    dsl.behavior && typeof dsl.behavior === "object" ? dsl.behavior : {}
  ) as Record<string, unknown>;
  const behavior = {
    autoRefresh: Boolean(rawBehavior.autoRefresh),
    refreshInterval:
      typeof rawBehavior.refreshInterval === "number"
        ? Math.max(5, rawBehavior.refreshInterval)
        : undefined,
    interactive:
      rawBehavior.interactive !== undefined
        ? Boolean(rawBehavior.interactive)
        : true,
  };
