...
import { getErrorMessage, getErrorResponseData, getErrorStatus } from '../../utils/errors'
...
const METHOD_PALETTE: Record<string, 'success' | 'primary' | 'warning' | 'error' | 'info'> = {
  GET: 'success',
  POST: 'primary',
  PUT: 'warning',
  DELETE: 'error',
...
function MethodBadge({ method, size = 'medium' }: { method: string; size?: 'small' | 'medium' }) {
  ...
  const colorKey = METHOD_PALETTE[method]
  const color = colorKey ? theme.palette[colorKey].main : theme.palette.text.secondary
  ...
  return (
    ...
    >
      <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1400, mx: 'auto' }}>
        <AdminPageHeader
          icon={<ApiIcon />}
          title={t('auth.developer.apiExplorer', 'API Explorer')}
          description={t(
