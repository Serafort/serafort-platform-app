...
export default function MFAVerificationScreen() {
  ...
  const handlePasskeyAuth = async () => {
    try {
      ...
      setError(passkeyError || err.response?.data?.message || err.message || 'Passkey verification failed.')
    }
  }
    ...
    >
      ...
      <Typography variant="body1" color="text.secondary" sx={{ fontWeight: 500, mb: 3, lineHeight: 1.6 }}>
        {activeMethod === 'recovery'
          ...
          : activeMethod === 'passkey'
          ? t('mfa.passkeyDescription', 'Verify your device biometrics, security key, or Windows Hello.')
      ...
      {error && (
        <Alert
          severity="error"
          sx={{ mb: 3, borderRadius: 2, textAlign: 'left', '& .MuiAlert-message': { fontWeight: 600 } }}
        >
          {error}
        </Alert>
      )}
      ...
      >
        <Tab icon={<PhoneAndroid fontSize="small" />} iconPosition="start" label="TOTP App" value="totp" />
        <Tab icon={<Sms fontSize="small" />} iconPosition="start" label="SMS OTP" value="sms" />
        <Tab icon={<Fingerprint fontSize="small" />} iconPosition="start" label="Passkey" value="passkey" />
        <Tab icon={<VpnKey fontSize="small" />} iconPosition="start" label="Backup Code" value="recovery" />
      </Tabs>
        ...
        {activeMethod === 'passkey' && (
          <Box sx={{ py: 2 }}>
            <Button
              ...
              startIcon={isPasskeyLoading ? <CircularProgress size={20} color="inherit" /> : <Fingerprint />}
              sx={{
                py: 1.8,
                borderRadius: 3,
                fontWeight: 800,
            ...
            >
              {isPasskeyLoading ? t('mfa.verifying', 'Checking Passkey...') : t('mfa.usePasskeyBtn', 'Sign in with Passkey')}
            </Button>
          </Box>
        )}
