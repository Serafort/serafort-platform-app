...
export default function ExportVerification() {
  ...
  const icons = {
    request: <CloudDownload sx={{ fontSize: 32 }} />,
    verifying: <LockOutlined sx={{ fontSize: 32 }} />,
    ready: <VerifiedUser sx={{ fontSize: 32 }} />,
  }
  const colors = {
    request: theme.palette.primary.main,
    verifying: theme.palette.primary.main,
    ...
    >
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3 }}>
        <Avatar
          ...
          sx={{
            ...
            border: '2px solid',
            color: colors[step],
            borderColor: alpha(colors[step], 0.2),
          }}
        >
          {icons[step]}
        </Avatar>
      ...
      >
        {step === 'request' &&
          t(
            'export.requestDesc',
            'Your account data is ready for export. For your protection, we require one-time verification.',
          )}
        {step === 'verifying' &&
          t('export.verifyingDesc', "We've sent a 6-digit code to your registered email address.")}
        {step === 'ready' &&
          t(
            'export.readyDesc',
            'Verification successful. Your secure download link is now active for 1 hour.',
          )}
      </Typography>
        ...
        {step === 'ready' && (
          <Button
            ...
            sx={{
              ...
              fontSize: '1rem',
              textTransform: 'none',
              boxShadow: (t) => `0 4px 14px ${alpha(t.palette.success.main, 0.4)}`,
            }}
          >
