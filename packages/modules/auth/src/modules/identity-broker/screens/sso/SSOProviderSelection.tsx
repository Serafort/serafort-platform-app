...
export default function SSOProviderSelection() {
  ...
  useDebounce(
    () => {
      if (isValidEmail) setDebouncedEmail(email)
  ...
  const handleContinue = useCallback(() => {
    if (detectedProvider?.url) {
      ...
      navigate(
        `${Path.identity.samlSSOInitiation}?domain=${emailDomain}${detectedProvider.organizationId ? `&organizationId=${detectedProvider.organizationId}` : ''}`,
      )
              ...
              <Grid key={p.id} size={{ xs: 12, sm: 4 }}>
                <Button
                  ...
                  sx={{
                    ...
                    '&:hover': {
                      bgcolor: alpha(p.color, theme.palette.mode === 'dark' ? 0.12 : 0.06),
