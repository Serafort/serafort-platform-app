...
export const DecisionNodeComponent = memo(({ data, selected }: NodeProps<DecisionNode>) => {
  const theme = useTheme()
  const isAllow = data.effect === 'allow'
  const effectColor = isAllow ? theme.palette.success.main : theme.palette.error.main
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
        ...
        >
          {isAllow ? <CheckCircleIcon sx={{ fontSize: 20 }} /> : <BlockIcon sx={{ fontSize: 20 }} />}
        </Box>
