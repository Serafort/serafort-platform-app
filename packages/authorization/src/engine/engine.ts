...
export class PolicyEngine {
  ...
  private matchesResource(rule: PolicyRule, resource: PolicyResource): boolean {
    ...
    return (
      rule.resources.includes(resource.type) || rule.resources.includes("*")
    );
  }

  private matchesRoleOrPermission(
    rule: PolicyRule,
    ...
    if (hasRoleRequirement) {
      roleMatched = rule.roles!.some(
        (role) =>
          subject.roles.includes(role) || subject.roles.includes("super_admin"),
      );
    }

    let permMatched = false;
    if (hasPermRequirement) {
      permMatched = rule.permissions!.some((perm) =>
  ...
  private matchesCondition(
    ...
    if (!evaluator) {
      if (import.meta.env?.DEV)
        console.warn(
          `[PolicyEngine] Condition evaluator '${rule.condition.id}' not found. Defaulting to false.`,
        );
      return false;
    }
