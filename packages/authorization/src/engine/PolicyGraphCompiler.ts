...
export class PolicyGraphCompiler {
  ...
  public static compileGraphToPolicySet(graph: VisualPolicyGraph): PolicySet {
    ...
    const nonSubjectEntries = nodes.filter(
      (n) => n.type !== 'subject' && n.type !== 'decision' && !edges.some((e) => e.target === n.id)
    )
    ...
    const policy: Policy = {
      ...
      description: graph.description || name || 'Compiled Visual ABAC/RBAC Policy',
      rules: compiledRules,
    }
   ...
   */
  private static buildRuleFromPath(path: PolicyCanvasNode[]): PolicyRule | null {
    ...
    for (const node of path) {
      if (node.type === 'subject') {
        ...
        if (data.permissions && data.permissions.length > 0) permissions = [...data.permissions]
      } else if (node.type === 'action') {
        const data = node.data as ActionNodeData
        if (data.actions && data.actions.length > 0) actions = [...data.actions]
    ...
    return {
      ...
      actions,
      resources,
      condition: conditionId ? { id: conditionId, args: conditionArgs } : undefined,
      priority,
    }
  }
   ...
   */
  public static decompilePolicySetToGraph(policySet: PolicySet): VisualPolicyGraph {
  ...
  public static simulateGraph(
    ...
    const evaluatePath = (currentNodeId: string, currentPath: PolicyCanvasNode[]): boolean => {
      const node = nodeMap.get(currentNodeId)
      ...
      if (node.type === 'subject') {
        const data = node.data as SubjectNodeData
        const hasRoleReq = data.roles && data.roles.length > 0 && !data.roles.includes('*')
        const hasPermReq = data.permissions && data.permissions.length > 0
        const rolesMatch = !hasRoleReq || data.roles.some((r) => subject.roles.includes(r) || subject.roles.includes('super_admin') || subject.roles.includes('ADMIN'))
        const permsMatch = !hasPermReq || data.permissions!.some((p) => subject.permissions.includes(p))
        ...
        const data = node.data as ResourceNodeData
        passed = data.resourceType === '*' || data.resourceType === resource.type
        message = passed
    ...
    if (successfulCandidates.length > 0) {
      ...
      successfulCandidates.sort((a, b) => {
        ...
        if (a.decision.effect === 'deny' && b.decision.effect === 'allow') return -1
        if (a.decision.effect === 'allow' && b.decision.effect === 'deny') return 1
        return 0
      })

      const winningCandidate = successfulCandidates[0]
      const winningDecisionNode = winningCandidate.path[winningCandidate.path.length - 1]
      matchedDecisionNodeId = winningDecisionNode.id
      resolvedEffect = winningCandidate.decision.effect
      resolvedReason = winningCandidate.decision.reason || `Evaluated to ${resolvedEffect.toUpperCase()} via visual rule path`
      ...
      for (let i = 0; i < winningCandidate.path.length; i++) {
        ...
        if (i < winningCandidate.path.length - 1) {
          ...
          const matchingEdge = edges.find((e) => e.source === node.id && e.target === nextNode.id)
  ...
  public static validateGraph(graph: VisualPolicyGraph): {
    ...
    if (!hasSubject) warnings.push('Graph has no Subject nodes. Rules may match all principals.')
    if (!hasDecision) errors.push('Graph must contain at least one Decision (ALLOW/DENY) node.')
