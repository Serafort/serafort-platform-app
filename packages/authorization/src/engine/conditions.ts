...
import { getRoleRank, isPlatformRole, isTenantRole, normalizeRole } from '@cap/shared-types'
...
registerCondition('sameOrg', (subject: PolicySubject, resource: PolicyResource) => {
  const resourceOrg = resource.attributes?.orgId ?? resource.attributes?.organizationId ?? resource.attributes?.tenantId
  // If resource has no org scoping specified, allow
  if (resourceOrg === undefined) return true

  const subjectOrg = subject.attributes?.orgId ?? subject.attributes?.organizationId ?? subject.activeTenantId
...
registerCondition('isPlatformUser', (subject: PolicySubject) => {
  if (subject.attributes?.isPlatform === true || subject.plane === 'platform') return true
...
registerCondition('isTenantOwner', (subject: PolicySubject, resource: PolicyResource) => {
  ...
  if (subject.memberships && subject.activeTenantId) {
    const mem = subject.memberships.find((m) => String(m.orgId) === String(subject.activeTenantId))
    if (mem?.isOwner || normalizeRole(mem?.role) === 'tenant_owner') return true
  }
  // Check resource ownerId match
  if (resource.attributes?.ownerId && String(subject.id) === String(resource.attributes.ownerId)) return true
  return false
})
...
registerCondition('isNotImpersonationReadOnly', (subject: PolicySubject) => {
  if (!subject.attributes?.isImpersonating && !subject.impersonationSession) return true
  return subject.attributes?.impersonationReadOnly !== true && subject.impersonationSession?.readOnly !== true
})
...
registerCondition('hasAllowedRoles', (subject: PolicySubject, resource: PolicyResource) => {
  ...
  const normalizedAllowed = allowedRoles.map((r) => normalizeRole(r)).filter(Boolean) as string[]
