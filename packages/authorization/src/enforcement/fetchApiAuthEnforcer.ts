...
import { PolicyAction, PolicyResource, PolicyActionEnum } from '../types/policy.types'
...
export interface FetchAuthEnforcerOptions {
   ...
   */
  getResource?: (endpoint: string, config: FetchRequestLike) => PolicyResource | undefined
  ...
  onDenied?: (info: { endpoint: string; action: PolicyAction; resource: PolicyResource }) => void
}
 ...
 */
export function createFetchApiAuthEnforcer(options: FetchAuthEnforcerOptions = {}): FetchAuthEnforcer {
  const defaultGetAction = (method?: string): PolicyAction => {
    const m = (method || 'GET').toUpperCase()
    if (m === HttpMethodEnum.GET || m === 'HEAD') return PolicyActionEnum.READ
    if (m === HttpMethodEnum.POST || m === HttpMethodEnum.PUT || m === HttpMethodEnum.PATCH) return PolicyActionEnum.WRITE
  ...
  const beforeRequest: FetchAuthEnforcer['beforeRequest'] = (endpoint, config) => {
