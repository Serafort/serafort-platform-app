...
export function buildSubject(userRaw: any): PolicySubject | null {
  ...
  const activeTenantId = userRaw.activeTenantId ?? user.activeTenantId ?? user.organizationId ?? user.orgId
  ...
  const isImpersonatingActive =
    !!impersonationSession &&
    (!impersonationSession.expiresAt || impersonationSession.expiresAt > Date.now())

  // Resolve base role
  let baseRole = normalizeRole(user.role) || normalizeRole(user.roleObject) || normalizeRole(user.roleName)
  let activeMembership: TenantMembership | undefined

  if (activeTenantId && memberships.length > 0) {
    activeMembership = memberships.find((m) => String(m.orgId) === String(activeTenantId))
  ...
  // Resolve plane
  const plane: RolePlane = user.plane || (isPlatformRole(baseRole) ? 'platform' : 'tenant')
  ...
  const membershipPerms: string[] = activeMembership && Array.isArray(activeMembership.permissions)
    ? activeMembership.permissions
    : []

  const permissionsSet = new Set<string>([...defaultPerms, ...userPerms, ...membershipPerms])

  const effectiveOrgId = isImpersonatingActive
    ? impersonationSession.targetOrgId
    : activeTenantId || user.organizationId || user.orgId || user.roleObject?.organizationId
  ...
  const attributes: Record<string, unknown> = {
    ...
    impersonationSessionId: isImpersonatingActive ? impersonationSession.sessionId : undefined,
    impersonationReadOnly: isImpersonatingActive ? impersonationSession.readOnly : undefined,
