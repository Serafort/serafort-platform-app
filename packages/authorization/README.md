...
#### `usePolicy()`
Returns the full `PolicyDecision` object if you need to know *why* an action was denied (e.g., to show a specific error message).
...
You can intercept and block unauthorized API requests at the network layer *before* they leave the browser. Use `createFetchApiAuthEnforcer` to create a hook that integrates with your `fetch` client.
