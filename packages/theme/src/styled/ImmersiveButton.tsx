...
const StyledImmersiveButton = styled("button")<
  Omit<ImmersiveButtonProps, "children">
>(({
  theme,
  perspective,
  rotationX,
  ...
  const resolvedDepth =
    depth ?? theme.tenantTheme?.effects.immersive?.depth ?? 10;
  const resolvedShadowColor =
    shadowColor ||
    theme.tenantTheme?.effects.immersive?.shadowColor ||
    "rgba(0,0,0,0.2)";

  return {
    perspective:
      perspective ||
      theme.tenantTheme?.effects.immersive?.perspective ||
      immersiveStyles.perspective,
