...
const StyledOrganicButton = styled("button")<
  Omit<OrganicButtonProps, "children">
>(({ theme, curvature, fluidity, backgroundColor, borderColor, padding }) => {
  const organicStyles = resolveOrganicThemeStyles(theme);
  const resolvedCurvature =
    curvature ?? theme.tenantTheme?.effects.organic?.curvature ?? 80;
  const resolvedFluidity =
    fluidity ?? theme.tenantTheme?.effects.organic?.fluidity ?? 50;

  return {
    borderRadius: getOrganicRadius(resolvedCurvature),
    background:
      backgroundColor || organicStyles.background || theme.palette.primary.main,
    ...
    "&:hover": {
      ...
      boxShadow:
        (typeof theme.customShadows?.primary === "object"
          ...
          : theme.customShadows?.primary) || theme.shadows[4],
    },
    "&:active": {
