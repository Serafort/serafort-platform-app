...
import { Box, Stack, Typography, Paper, Divider, Button, Avatar, Chip, Card, CardContent, Grid, Snackbar, Alert } from '@mui/material'
...
const DynamicLayoutWidget: React.FC<DynamicLayoutWidgetProps> = ({ nodes }) => {
  ...
  const renderNode = (node: WidgetRenderNode, index: number): React.ReactNode => {
    ...
    const actionProps = action ? {
      ...
      style: { cursor: 'pointer', ...(props.style as React.CSSProperties || {}) }
    } : {}
    ...
    const renderChildren = () => {
      ...
      return children.map((child: WidgetRenderNode, i: number) => renderNode(child, i))
    }

    switch (type) {
      case 'box':
        return <Box key={key} {...combinedProps}>{renderChildren()}</Box>
      case 'stack':
        return <Stack key={key} {...combinedProps}>{renderChildren()}</Stack>
      case 'paper':
        return <Paper key={key} {...combinedProps}>{renderChildren()}</Paper>
      case 'typography':
        return <Typography key={key} {...combinedProps}>{renderChildren()}</Typography>
      case 'divider':
        return <Divider key={key} {...combinedProps} />
      case 'button':
        return <Button key={key} {...combinedProps}>{renderChildren()}</Button>
      case 'avatar':
        return <Avatar key={key} {...combinedProps}>{renderChildren()}</Avatar>
      case 'chip':
      ...
      case 'grid':
        return <Grid key={key} {...combinedProps}>{renderChildren()}</Grid>
      case 'icon': {
        ...
        if (nameStr && nameStr in MuiIcons) {
          const IconComponent = MuiIcons[nameStr as keyof typeof MuiIcons] as React.ElementType
      ...
      default:
        return <Box key={key} p={1} bgcolor="error.main">Unknown node type: {type}</Box>
    }
  }
  ...
  return (
    <Box sx={{ width: '100%', height: '100%', overflow: 'auto', position: 'relative' }}>
      ...
      >
        <Alert onClose={() => setFeedback(null)} severity="info" sx={{ width: '100%' }}>
          {feedback}
        </Alert>
      </Snackbar>
    </Box>
