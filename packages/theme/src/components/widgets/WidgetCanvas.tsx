...
const EmptySlotDroppableInner: React.FC<{
  slotId: string;
  layoutId: string;
  sizeConfig?: SlotSizeConfig;
  effectiveMode: DashboardMode;
}> = ({ slotId, layoutId, sizeConfig, effectiveMode }) => {
  ...
  const handleNativeDrop = (e: React.DragEvent) => {
    ...
    if (widgetId) {
      if (import.meta.env.DEV)
        console.log("[Native DND Drop on Empty Slot]", {
          widgetId,
          layoutId,
          slotId,
        });
  ...
  if (isOver && import.meta.env.DEV) {
    console.log("[WidgetCanvas] Hovering over EmptySlotDroppable:", {
      layoutId,
      slotId,
    });
  }
    ...
    >
      <Box
        sx={{
          ...
          minHeight: sizeConfig?.height || 280,
          border: (theme) =>
            `2px dashed ${isHighlighted ? theme.palette.primary.main : theme.palette.divider}`,
          ...
          boxShadow: isHighlighted
            ? (theme) => `0 0 16px ${theme.palette.primary.main}33`
            : "none",
        }}
      >
        <Typography
        ...
        {effectiveMode === "custom" && (
          <Tooltip title="Remove Empty Panel Slot">
            <IconButton
              size="small"
              color="error"
              aria-label={"Remove empty slot " + slotId}
              onClick={() => {
                if (import.meta.env.DEV)
      ...
      <SortableContext items={sortableItemIds} strategy={rectSortingStrategy}>
        ...
        >
          {uniqueSlots.map((slotId: string) => {
            const slotValue: SlotWidgetValue | undefined =
              layout.slotWidgets[slotId];
            const widgetId = getWidgetIdFromSlotValue(slotValue);
            const nodeSubLayout = getSubLayoutFromSlotValue(slotValue);
            const nodeSize = isWidgetNode(slotValue)
              ? slotValue.size
              : undefined;
            const sizeConfig =
              nodeSize || (layout.slotSizes && layout.slotSizes[slotId]);
            const descriptor = widgetId
              ? globalWidgetRegistry.get(widgetId)
              : undefined;

            const config = isWidgetNode(slotValue)
              ? slotValue.config
              : undefined;
