...
const WidgetWrapperInner: React.FC<WidgetWrapperProps> = ({
  ...
  const slots = useAppStore(
    (state) => state.layouts[pageId]?.slots || EMPTY_SLOTS,
  );
  ...
  const availableLayouts = useAppStore(
    useShallow((state) =>
      Object.keys(state.layouts || {}).filter((id) => id !== pageId),
    ),
  );
  ...
  const handleMoveLeft = useCallback(() => {
    if (import.meta.env.DEV)
      console.log("[WidgetWrapper] Menu MoveLeft:", { pageId, slotId });
  ...
  const handleMoveRight = useCallback(() => {
    if (import.meta.env.DEV)
      console.log("[WidgetWrapper] Menu MoveRight:", { pageId, slotId });
    actions.moveWidgetBy(pageId, slotId, 1);
    setMenuAnchor(null);
  }, [actions, pageId, slotId]);

  const handleResizeSpan = useCallback(
    (span: WidgetSpan) => {
      if (import.meta.env.DEV)
        console.log("[WidgetWrapper] Menu ResizeSpan:", {
          pageId,
          slotId,
          span,
        });
  ...
  const handleResizeHeight = useCallback(
    (height: WidgetHeight) => {
      if (import.meta.env.DEV)
        console.log("[WidgetWrapper] Menu ResizeHeight:", {
          pageId,
          slotId,
          height,
        });
  ...
  const handleReset = useCallback(() => {
    if (import.meta.env.DEV)
      console.log("[WidgetWrapper] Menu Reset:", { pageId, slotId });
    if (onReset) {
      onReset();
  ...
  const handleTransfer = useCallback(
    (targetLayoutId: string, targetSlotId: string) => {
      if (import.meta.env.DEV)
        console.log("[WidgetWrapper] Menu Transfer:", {
          pageId,
          slotId,
          targetLayoutId,
          targetSlotId,
  ...
  const handleRemoveWidget = useCallback(() => {
    if (import.meta.env.DEV)
      console.log("[WidgetWrapper] Menu RemoveWidget:", { pageId, slotId });
  ...
  const handleRemovePanel = useCallback(() => {
    if (import.meta.env.DEV)
      console.log("[WidgetWrapper] Menu RemovePanel:", { pageId, slotId });
  ...
  const handleNativeDrop = (e: React.DragEvent) => {
    ...
    if (dropWidgetId) {
      if (import.meta.env.DEV)
        console.log("[Native DND Drop on WidgetWrapper]", {
          dropWidgetId,
          pageId,
          slotId,
        });
      actions.transferWidget(
        "marketplace-catalog",
        `catalog-${dropWidgetId}`,
        pageId,
        slotId,
      );
    }
  ...
  const containerDragProps =
    mode === "custom" ? { ...attributes, ...listeners } : {};
  ...
  return (
    <Box
      ...
      sx={{
        ...
        borderRadius: 2,
        cursor:
          mode === "custom" ? (isDragging ? "grabbing" : "grab") : "default",
        transition: "border-color 0.2s, box-shadow 0.2s",
        ...(mode === "custom" && {
          border: (theme) =>
            `1px dashed ${isDragging ? theme.palette.primary.main : theme.palette.primary.light}`,
          "&:hover": {
        ...
        >
          ...
          >
            ...
            >
              ...
              <ListItemText>
                {t("dashboard.inspectWidget", {
                  defaultValue: "Configure & Inspect",
                })}
              </ListItemText>
            </MenuItem>
            <Divider />
            ...
            {availableLayouts.length > 0 && (
              ...
              >
                {t("dashboard.moveToCanvas", {
                  defaultValue: "Move to Canvas",
                })}
              </ListSubheader>
            )}
            {availableLayouts.map((targetLayoutId) => (
              <MenuItem
                key={`tr-${targetLayoutId}`}
                onClick={() =>
                  handleTransfer(targetLayoutId, `${targetLayoutId}-slot-1`)
                }
              >
                <ListItemIcon>
            ...
            <ListSubheader
              component="div"
              sx={{ lineHeight: "28px", fontSize: "0.75rem", fontWeight: 700 }}
            >
              {t("dashboard.resizeWidth")}
            </ListSubheader>
            <MenuItem
              selected={slotSize.span === 4}
              onClick={() => handleResizeSpan(4)}
            >
              <ListItemIcon>
                {slotSize.span === 4 ? (
            ...
            <MenuItem
              selected={slotSize.span === 8}
              onClick={() => handleResizeSpan(8)}
            >
              <ListItemIcon>
                {slotSize.span === 8 ? (
                  <Check fontSize="small" />
            ...
            <MenuItem
              selected={slotSize.span === 12}
              onClick={() => handleResizeSpan(12)}
            >
              <ListItemIcon>
                {slotSize.span === 12 ? (
                  <Check fontSize="small" />
            ...
            <MenuItem
              selected={slotSize.height === 200}
              onClick={() => handleResizeHeight(200)}
            >
              <ListItemIcon>
                {slotSize.height === 200 ? (
                  <Check fontSize="small" />
            ...
            <MenuItem
              selected={slotSize.height === 280}
              onClick={() => handleResizeHeight(280)}
            >
              <ListItemIcon>
                {slotSize.height === 280 ? (
                  <Check fontSize="small" />
            ...
            <MenuItem
              selected={slotSize.height === 400}
              onClick={() => handleResizeHeight(400)}
            >
              <ListItemIcon>
                {slotSize.height === 400 ? (
                  <Check fontSize="small" />
        ...
        <React.Suspense
          fallback={
            <Skeleton
              variant="rounded"
              sx={{ flex: 1 }}
              aria-label={t("dashboard.widgetLoading")}
            />
          }
        >
              ...
              fallback={
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{ fontSize: "0.8rem" }}
                  >
                    {t("dashboard.widgetLoadError", {
                      defaultValue: "Widget failed to render",
                    })}
                  </Typography>
                  <Button
                    size="small"
