...
export const getHorizontalMenuPopoutOffset = (level?: number): number => {
  return level && level > 0
    ? layoutMenuTokens.horizontalMenu.popoutOffsetSublevel
    : layoutMenuTokens.horizontalMenu.popoutOffsetLevel0;
};

/**
 * Returns mode-aware hover background string for horizontal menu items
 */
export const getHorizontalMenuButtonHoverBg = (
  theme: Theme,
  active?: boolean,
): string => {
  return active
    ? getPrimaryMainOpacity(theme)
    : `${alpha(theme.palette.text.primary, layoutMenuTokens.horizontalMenu.button.hoverBgAlpha)} !important`;
};
...
export const getNavbarIconButtonHoverBg = (theme: Theme): string => {
  return alpha(
    ...
    layoutMenuTokens.navbarContent.iconButtonHoverAlpha,
  );
};
