...
export type SurfaceEffectBuilder = (
  config: EffectConfig,
  theme?: Theme,
) => CSSObject;
...
const buildGlassEffect: SurfaceEffectBuilder = (config, theme) => {
  ...
  const borderWidthToken =
    (theme as any)?.tenantTheme?.tokens?.borderWidth?.thin || "1px";
  return {
    backdropFilter:
      glassStyles.backdropFilter || `blur(${glass.blur || "16px"})`,
    background:
      glass.background ||
      (theme
        ? alpha(paperColor, glass.opacity ?? 0.88)
        : `rgba(255, 255, 255, ${glass.opacity ?? 0.88})`),
    borderColor:
      glass.borderColor || theme?.palette?.divider || "rgba(0, 0, 0, 0.12)",
    borderStyle: "solid",
...
const buildLiquidGlassEffect: SurfaceEffectBuilder = (config, theme) => {
  ...
  return {
    ...
    border:
      liquidStyles.border ||
      `${liquid.borderWidth || borderWidthToken} solid ${liquid.borderColor || "rgba(255, 255, 255, 0.2)"}`,
...
const buildBrutalismEffect: SurfaceEffectBuilder = (config, theme) => {
  ...
  const borderWidthToken =
    (theme as any)?.tenantTheme?.tokens?.borderWidth?.medium || "2px";
  return {
    background:
      brutal.backgroundColor || theme?.palette?.background?.paper || "#ffffff",
    border:
      brutalStyles.border ||
      `${brutal.borderWidth || borderWidthToken} solid ${brutal.borderColor || fallbackColor}`,
    boxShadow:
      brutalStyles.boxShadow ||
      `${brutal.shadowOffset || "4px"} ${brutal.shadowOffset || "4px"} 0px ${brutal.shadowColor || fallbackColor}`,
  };
};
...
const buildBentoEffect: SurfaceEffectBuilder = (config, theme) => {
  ...
  return {
    background:
      bento.background || theme?.palette?.background?.paper || "#ffffff",
    borderRadius:
      bento.borderRadius ||
      (theme ? `${theme.shape.borderRadius * 2}px` : "24px"),
    border:
      bentoStyles.border ||
      `${bento.borderWidth || borderWidthToken} solid ${bento.borderColor || fallbackBorder}`,
 ...
 */
export const buildSurfaceEffect = (
  config: EffectConfig,
  theme?: Theme,
): CSSObject => {
