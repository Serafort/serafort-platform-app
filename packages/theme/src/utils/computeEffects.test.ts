...
describe('computeEffects atomic and composite utilities', () => {
  ...
  describe('Neumorphism Atomics & Composites', () => {
    ...
    it('computes box shadows for normal, pressed and disabled states', () => {
      ...
      const normal = computeNeumorphismBoxShadow({ enabled: true, distance: 4 });
      expect(normal).not.toContain('inset');

      const pressed = computeNeumorphismBoxShadow({ enabled: true, distance: 4 }, true);
    ...
    it('computes neumorphism background with fallbacks', () => {
      ...
      expect(computeNeumorphismBackground(undefined, lightTheme)).toBe('#ffffff');
  ...
  describe('Glassmorphism Atomics & Composites', () => {
    ...
    it('computes glass background with theme, hex parsing, and fallback', () => {
      ...
      expect(computeGlassBackground('#ff0000', 0.5)).toBe('rgba(255, 0, 0, 0.5)');
      expect(computeGlassBackground(undefined, 0.6, lightTheme)).toContain('rgba(255, 255, 255, 0.6)');
      expect(computeGlassBackground(undefined, 0.8)).toBe('rgba(255, 255, 255, 0.1)');
    });

    it('computes glass border', () => {
      expect(computeGlassBorder('2px', 'red')).toBe('2px solid red');
      expect(computeGlassBorder('1px', undefined, lightTheme)).toBe('1px solid rgba(0, 0, 0, 0.12)');
      expect(computeGlassBorder()).toBe('1px solid rgba(255, 255, 255, 0.2)');
    });

    it('composes getGlassmorphismStyles correctly', () => {
      const styles = getGlassmorphismStyles({ blur: '12px', opacity: 0.7 }, lightTheme);
  ...
  describe('Liquid Glass Atomics & Composites', () => {
    it('computes liquid glass backdrop', () => {
      ...
      expect(computeLiquidGlassBackdrop('30px')).toBe('blur(30px) saturate(180%)');
    });
    ...
    it('computes liquid glass background for light, dark, hex, and theme', () => {
      ...
      expect(computeLiquidGlassBackground('#00ff00', 0.5)).toBe('rgba(0, 255, 0, 0.5)');
      expect(computeLiquidGlassBackground(undefined, 0.8, true, lightTheme)).toContain('rgba(255, 255, 255, 0.8)');
      expect(computeLiquidGlassBackground(undefined, undefined, true)).toBe('rgba(255, 255, 255, 0.82)');
      expect(computeLiquidGlassBackground(undefined, undefined, false)).toBe('rgba(23, 23, 31, 0.75)');
    });

    it('computes liquid glass border, inner shadow, specular highlight, and combined shadow', () => {
      expect(computeLiquidGlassBorder('1px', undefined, true)).toBe('1px solid rgba(255, 255, 255, 0.5)');
      expect(computeLiquidGlassBorder('1px', undefined, false)).toBe('1px solid rgba(255, 255, 255, 0.12)');
      expect(computeLiquidGlassInnerShadow(undefined, true)).toContain('inset 0 1px 1px 0 rgba(255, 255, 255, 0.8)');
      expect(computeLiquidGlassSpecularHighlight(undefined, true)).toContain('0 12px 36px 0 rgba(31, 38, 135, 0.14)');
  ...
  describe('Brutalism Atomics & Composites', () => {
    it('computes brutalism border, shadow, and background', () => {
      expect(computeBrutalismBorder('3px', '#ff0000')).toBe('3px solid #ff0000');
      expect(computeBrutalismBorder('2px', undefined, lightTheme)).toBe('2px solid #111827');
      expect(computeBrutalismBorder()).toBe('2px solid #000000');

      expect(computeBrutalismShadow('6px', '#333333')).toBe('6px 6px 0px 0px #333333');
      expect(computeBrutalismShadow('4px', undefined, darkTheme)).toBe('4px 4px 0px 0px #f8fafc');
    ...
    it('composes getBrutalismStyles correctly', () => {
      const styles = getBrutalismStyles({ borderWidth: '3px', shadowOffset: '5px' }, lightTheme);
  ...
  describe('Bento Atomics & Composites', () => {
    it('computes bento radius, background, border, and shadow', () => {
      ...
      expect(computeBentoBorder('1px', undefined, lightTheme)).toBe('1px solid rgba(0, 0, 0, 0.12)');

      expect(computeBentoShadow('custom-shadow')).toBe('custom-shadow');
      expect(computeBentoShadow(undefined, lightTheme)).toBe(lightTheme.shadows[4]);
  ...
  describe('Organic Atomics & Composites', () => {
    ...
    it('composes getOrganicStyles correctly', () => {
      const styles = getOrganicStyles({ curvature: 70, fluidity: 50 }, lightTheme);
  ...
  describe('Immersive Atomics & Composites', () => {
    it('computes immersive perspective, transform, shadow, and background', () => {
      ...
      expect(computeImmersiveTransform('5deg', '10deg')).toBe('rotateX(5deg) rotateY(10deg)');
      expect(computeImmersiveTransform()).toBe('rotateX(0deg) rotateY(0deg)');

      const shadow = computeImmersiveShadow(30, 'rgba(0,0,0,0.5)');
      expect(shadow).toBe('0 7.5px 15px rgba(0,0,0,0.5), 0 30px 45px rgba(0,0,0,0.5)');
    ...
    it('composes getImmersiveStyles correctly', () => {
      const styles = getImmersiveStyles({ rotationX: '15deg', rotationY: '-10deg', depth: 20 }, lightTheme);
      ...
      expect(styles.transition).toBe('transform 0.3s ease-out, box-shadow 0.3s ease-out');
    });
  });
  ...
  describe('Color & RGBA Utilities', () => {
    ...
    it('applies alphaColor across various color inputs and clamps alpha', () => {
      ...
      expect(alphaColor('rgb(255, 255, 255)', 0.8)).toBe('rgba(255, 255, 255, 0.8)');
      expect(alphaColor('rgba(255, 0, 0, 0.2)', 0.9)).toBe('rgba(255, 0, 0, 0.9)');
