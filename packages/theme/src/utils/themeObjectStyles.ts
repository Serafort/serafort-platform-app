...
export const getTenantComponentConfig = (
  ...
  getTenantThemeConfig(theme).components?.[componentName] ||
  DEFAULT_COMPONENT_STYLES[componentName];

export const resolveComponentCustomProperties = (
  theme: Theme,
  componentName: keyof ComponentStyles,
): CSSProperties => {
  const customProperties = getTenantComponentConfig(
    theme,
    componentName,
  ).customProperties;
...
export const resolveBrutalismThemeStyles = (theme: Theme) =>
  getBrutalismStyles(
    getTenantThemeEffects(theme).brutalism || DEFAULT_EFFECT_CONFIG.brutalism!,
    theme,
  );

export const resolveBentoThemeStyles = (theme: Theme) =>
  getBentoStyles(
    getTenantThemeEffects(theme).bento || DEFAULT_EFFECT_CONFIG.bento!,
    theme,
  );

export const resolveOrganicThemeStyles = (theme: Theme) =>
  getOrganicStyles(
    getTenantThemeEffects(theme).organic || DEFAULT_EFFECT_CONFIG.organic!,
    theme,
  );

export const resolveImmersiveThemeStyles = (theme: Theme) =>
  getImmersiveStyles(
    getTenantThemeEffects(theme).immersive || DEFAULT_EFFECT_CONFIG.immersive!,
    theme,
  );

export const getThemeFocusRing = (
  theme: Theme,
...
export const getThemeBorderRadius = (
  theme: Theme,
  fallback = theme.shape.borderRadius,
) => {
