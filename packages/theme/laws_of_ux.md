...
A comprehensive list of user experience design principles, psychological heuristics, and engineering best practices across the Serafort Multi-Tenant Framework.

---

## 4 Key Principles Guiding UI

Every interface and layout in the Serafort ecosystem is anchored in four foundational visual principles:
...
- **Serafort Implementation**: Enforced through `@cap/theme` presets, liquid glass visual effects, and fluid typography tokens (`--font-display` to `--font-body`).
...
- **Serafort Implementation**: Multi-tenant configuration screens present curated presets first; granular CSS variable controls are grouped into advanced collapsible panels.
...
- **Serafort Implementation**: Bento grid card layouts, multi-step stepper pipelines (`@cap/module-landing`), and grouped form sections.
...
- **Serafort Implementation**: Transparent subscription cancellation and explicit confirmation modals (`packages/theme/DESIGN_SYSTEM.md` §2).
...
- **Serafort Implementation**: Isolated widget error boundaries, self-explanatory button labels, and uncluttered navigation shells.
...
- **Serafort Implementation**: CSS transitions bounded to 150ms-300ms; button action locks (`100ms`); `ThemeBridge` single-frame RAF updates.
...
- **Serafort Implementation**: Form inputs enforce `minHeight: 48px`, buttons enforce minimum 44px touch targets (`var(--touch-target-min)`).
...
- **Serafort Implementation**: Zero layout shift (CLS < 0.1) enforced with `scrollbar-gutter: stable;` and fixed skeleton geometry.
...
- **Serafort Implementation**: Multi-step onboarding wizards and registration pipelines display numeric step indicators (e.g. "Step 3 of 4: Organization Setup").
...
- **Serafort Implementation**: Tenant onboarding defaults to recommended theme presets rather than asking for 40 raw color tokens upfront.
...
- **Serafort Implementation**: Standardized navigation layouts (`VerticalLayout`, `HorizontalLayout`) with top header utility area and standard keyboard command palette (`Cmd/Ctrl + K`).
...
- **Serafort Implementation**: Bento grid card architecture, surface elevation layers (`background.paper`, `borders.subtle`).
...
- **Serafort Implementation**: MUI spacing scale tokens (`theme.spacing(1)` for label-input gap, `theme.spacing(3)` for section separation).
...
- **Serafort Implementation**: Consistent SVG icon set, crisp typography, and uncluttered metric card widgets.
...
- **Serafort Implementation**: Global component style overrides (`getComponentOverrides`) applied uniformly across all modules.
...
- **Serafort Implementation**: Pipeline step connectors, timeline trees, and tab underline indicators.
...
- **Serafort Implementation**: User directory, session management, and auth configuration follow established industry IAM conventions.
...
- **Serafort Implementation**: Navigation items in module contracts (`navItems`) group entries into distinct titled sections with max 5-7 items per group.
...
- **Serafort Implementation**: Direct, single-click actions for low-risk actions; clear single-modal flows for destructive actions.
...
- **Serafort Implementation**: 4 UI States architecture with guided Empty States (`.state-empty-container`) featuring instant "Create First Item" CTAs.
...
- **Serafort Implementation**: Command palette (`SearchItemConfig`), quick-action headers, and top-level dashboard metrics.
...
- **Serafort Implementation**: Live form masks, autofill tokens, and instant client-side validation.
...
- **Serafort Implementation**: Success feedback states (`.state-success-glow`), rewarding confirmation screens, and clean session summaries.
...
- **Serafort Implementation**: Input formatting masks, resilient data parsers, and strictly typed API schemas (`@cap/api-contracts`).
...
- **Serafort Implementation**: One focal point per view, accessible dialog backdrops, and active menu item highlighting.
...
- **Serafort Implementation**: Home/Dashboard as first navigation item, Settings/Profile as last; primary CTA anchored at the end of action rows.
...
- **Serafort Implementation**: Multi-tenant token compilation (`composeMuiTheme`), dynamic module assembly (`assembleApp`), and automated RTL flipping handled transparently by the framework.
...
- **Serafort Implementation**: `variant="contained"` with primary brand color for the main CTA; accent badges for "Recommended" or "Popular" options.
...
- **Serafort Implementation**: Summary side-panels in checkout/onboarding, sticky headers in long tables, and contextual breadcrumbs.
...
- **Serafort Implementation**: Onboarding checklist widgets, tenant setup progress meters, and draft auto-saving indicators.
