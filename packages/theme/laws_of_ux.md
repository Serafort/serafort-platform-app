...
1. **Visual Hierarchy**: Guiding the eye to the most important information first.
   - *Application*: Establish unambiguous focal points using calibrated scale, font weight, spatial isolation, and elevation tiers.
2. **Contrast**: Making elements legible and distinct from one another.
   - *Application*: Enforce WCAG AA/AAA contrast ratios (4.5:1 text, 3:1 UI boundaries) and isolate primary CTAs from secondary/neutral elements.
3. **Alignment**: Creating visual order to reduce the user's mental effort.
   - *Application*: Lock components to consistent 8pt/4pt grids, maintain precise baseline alignments, and respect logical inline/block directions (including RTL).
4. **Proximity**: Grouping related elements to indicate that they share a function.
   - *Application*: Apply tight internal spacing (4-8px) between tightly coupled controls (e.g. label + input) and generous outer spacing (24-32px) between independent cards and sections.
