...
const isContainerWidget = (val: SlotWidgetValue | undefined): boolean => {
  ...
  return (
    widgetId === "dashboard-widget-splitPane" ||
    widgetId === "dashboard-widget-tabbedCanvas"
  );
};
...
const swapWidgets = (layout: GridLayout, a: SlotId, b: SlotId): GridLayout => {
  ...
  const replacementForA =
    valB && valB !== valA && !isContainerWidget(valB) ? valB : "";
...
> = (set, get) => ({
  ...
  moveWidget: (pageId, fromSlot, toSlot) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] moveWidget called:", {
          pageId,
          fromSlot,
          toSlot,
        });
      ...
      if (layout) {
        ...
        if (import.meta.env.DEV)
          console.log(
            "[Store:layoutEngine] moveWidget succeeded:",
            state.layouts[pageId].slotWidgets,
          );
      } else {
        console.warn(
          "[Store:layoutEngine] moveWidget failed - layout not found:",
          pageId,
        );
      }
    }),
  ...
  moveWidgetBy: (pageId, slot, direction) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] moveWidgetBy called:", {
          pageId,
          slot,
          direction,
        });
  ...
  applySlotItemMap: (pageId, entries) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] applySlotItemMap called:", {
          pageId,
          entries,
        });
      const layout = state.layouts[pageId];
  ...
  resetLayout: (pageId, defaultLayout) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] resetLayout called:", pageId);
      state.layouts[pageId] = { ...defaultLayout };
    }),
  ...
  transferWidget: (fromLayoutId, fromSlot, toLayoutId, toSlot) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] transferWidget called:", {
          fromLayoutId,
          fromSlot,
          toLayoutId,
          toSlot,
      ...
      if (fromLayoutId === toLayoutId) {
        ...
        if (import.meta.env.DEV)
          console.log(
            "[Store:layoutEngine] transferWidget (same layout swap) result:",
            state.layouts[fromLayoutId].slotWidgets,
          );
        return;
      }
      ...
      if (!toLayout) {
        console.warn(
          "[Store:layoutEngine] transferWidget failed - target layout missing:",
          toLayoutId,
        );
        return;
      }
      ...
      if (fromLayout) {
        ...
        fromLayoutId === "marketplace-catalog" ||
        fromSlot.startsWith("catalog-")
      ) {
      ...
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] transferWidget widgets:", {
          widgetA,
          widgetB,
        });
      if (!widgetA) {
        console.warn(
          "[Store:layoutEngine] transferWidget failed - source widget empty",
        );
        return;
      }
      ...
      if (widgetB && widgetB !== "" && widgetB !== widgetA) {
        if (import.meta.env.DEV)
          console.log(
            "[Store:layoutEngine] Target slot occupied; finding or creating empty slot in target layout",
          );
        const emptySlot = toLayout.slots.find(
          (sId) =>
            !toLayout.slotWidgets[sId] || toLayout.slotWidgets[sId] === "",
        );
        if (emptySlot) {
      ...
      const newToSlots = Array.from(
        new Set(isToSlotNew ? [...toLayout.slots, targetSlot] : toLayout.slots),
      );
  ...
  removeWidget: (pageId, slotId) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] removeWidget called:", {
          pageId,
          slotId,
        });
      const layout = state.layouts[pageId];
      ...
      if (import.meta.env.DEV)
        console.log(
          "[Store:layoutEngine] removeWidget completed:",
          state.layouts[pageId].slotWidgets,
        );
    }),
  ...
  removePanel: (pageId, slotId) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] removePanel called:", {
          pageId,
          slotId,
        });
      const layout = state.layouts[pageId];
      ...
      if (import.meta.env.DEV)
        console.log(
          "[Store:layoutEngine] removePanel completed:",
          state.layouts[pageId],
        );
    }),
  ...
  addPanel: (pageId) =>
    set((state) => {
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] addPanel called:", pageId);
      const layout = state.layouts[pageId];
      if (!layout) return;
      ...
      if (import.meta.env.DEV)
        console.log(
          "[Store:layoutEngine] addPanel completed:",
          state.layouts[pageId],
        );
    }),
  ...
  addSlot: (pageId, slotId, widgetId, size) =>
    set((state) => {
      ...
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] addSlot completed:", {
          pageId,
          slotId,
          widgetId,
        });
  ...
  saveView: (viewName) =>
    set((state) => {
      ...
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] saveView:", { viewName, snapshot });
    }),
  ...
  loadView: (viewName) =>
    set((state) => {
      ...
      state.activeViewName = viewName;
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] loadView:", { viewName });
    }),
  ...
  deleteView: (viewName) =>
    set((state) => {
      ...
      if (import.meta.env.DEV)
        console.log("[Store:layoutEngine] deleteView:", viewName);
    }),
