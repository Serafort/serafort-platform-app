...
export async function replayOfflineQueue() {
  ...
  console.log(
    `[OfflineSync] Replaying ${offlineQueue.length} queued mutations...`,
  );
  ...
  try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (response.status === 409) {
            console.warn(
              `[OfflineSync] Conflict (409) for ${entry.id}. Manual resolution required.`,
            );
        ...
        console.error(
          `[OfflineSync] Failed to replay ${entry.id}:`,
          err.message,
        );
        ...
        const currentRetry =
          (offlineQueue.find((e) => e.id === entry.id)?.retryCount || 0) + 1;
        if (currentRetry > 5) {
          console.error(
            `[OfflineSync] Entry ${entry.id} failed too many times. Removing.`,
          );
          removeFromOfflineQueue(entry.id);
          useAppStore.getState().addNotification({
    ...
    useAppStore.getState().addNotification({
      ...
      message:
        "An unexpected issue occurred while syncing pending offline actions.",
      type: "error",
    });
  }
}
