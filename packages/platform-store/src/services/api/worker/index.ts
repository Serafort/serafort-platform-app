 ...
 */
export function createSharedWorkerApiClient(
  workerScriptUrl?: string,
): SharedWorkerApiClientProxy | null {
  ...
  if (typeof window.SharedWorker !== "undefined") {
    try {
      ...
      if (import.meta.env.DEV) {
        console.log(
          "⚡ [SharedWorker API] Connected to SharedWorker (Cross-Tab Network Deduplication Active)",
        );
      }
      return sharedWorkerProxyInstance;
    } catch (err) {
      if (import.meta.env.DEV) {
        console.warn(
          "[SharedWorker API] SharedWorker creation failed, trying Dedicated Worker fallback:",
          err,
        );
      }
    }
  ...
  if (typeof window.Worker !== "undefined") {
    try {
      const dedicatedWorker = workerScriptUrl
        ...
        : new Worker(new URL("./api.shared-worker.ts", import.meta.url), {
            type: "module",
          });

      sharedWorkerProxyInstance = new SharedWorkerApiClientProxy(
        dedicatedWorker,
      );
      if (import.meta.env.DEV) {
        console.log(
          "⚡ [Worker API] Connected to Dedicated Worker (Off-thread execution active)",
        );
      }
      return sharedWorkerProxyInstance;
    } catch (err) {
      if (import.meta.env.DEV) {
        console.warn(
          "[Worker API] Dedicated Worker creation failed, falling back to main-thread fetch:",
          err,
        );
      }
    }
