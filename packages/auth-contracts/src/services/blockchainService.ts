...
export class BlockchainService {
  ...
  async issueCredential(payload: {
    ...
    return apiClient.post<VerifiableCredential>(
      API_ENDPOINTS.blockchain.issueCredential,
      payload,
    );
  }
  ...
  async listCredentials(): Promise<FetchResponse<VerifiableCredential[]>> {
    return apiClient.get<VerifiableCredential[]>(
      API_ENDPOINTS.blockchain.credentials,
    );
  }
  ...
  async getAuditLogs(): Promise<FetchResponse<BlockchainAuditLog[]>> {
    return apiClient.get<BlockchainAuditLog[]>(
      API_ENDPOINTS.blockchain.auditLogs,
    );
  }
}
