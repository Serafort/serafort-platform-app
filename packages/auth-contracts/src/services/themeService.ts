...
export class ThemeService {
  ...
  async generateTheme(
    ...
    return apiClient.post<ThemeGenerateResponse>(ENDPOINTS.themes.generate, request)
  }
  ...
  async getTenantTheme(
    ...
    return apiClient.get<TenantThemeResponse>(ENDPOINTS.themes.tenant, { headers })
  }
  ...
  async saveTenantTheme(
    orgId: string | number,
    data: { themeConfig: TenantThemeConfig; presetId?: string; isDark?: boolean }
   ...
   */
  async getPresets(): Promise<FetchResponse<{ presets: Record<string, unknown> }>> {
    return apiClient.get<{ presets: Record<string, unknown> }>(ENDPOINTS.themes.presets)
  }
}
