...
describe('Routes Registry & Path Helpers (Tier 0 SSOT)', () => {
  ...
  it('compilePath should substitute route parameters correctly', () => {
    ...
    expect(compilePath(samlRoute, { id: 'saml_99' })).toBe('/admin/sso/saml-metadata/saml_99')

    const multiParamTemplate = '/org/:orgId/user/:userId'
    expect(compilePath(multiParamTemplate, { orgId: 'org_1', userId: 'usr_2' })).toBe('/org/org_1/user/usr_2')
  })
  ...
  it('resolveDynamicPath should fallback to default path when item is not registered', () => {
    ...
    // Matched item
    expect(resolveDynamicPath(registered, 'dashboard', '/fallback-dashboard')).toBe('/custom-dashboard')

    // Unmatched item
    expect(resolveDynamicPath(registered, 'non-existent', AppPaths.landing.home)).toBe('/')
  })
})
