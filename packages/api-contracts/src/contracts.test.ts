...
describe("API_CONTRACTS registry", () => {
  ...
  it("gives every contract a supported HTTP method", () => {
    for (const contract of contracts) {
      expect(
        VALID_METHODS,
        `contract "${contract.id}" uses method ${contract.method}`,
      ).toContain(contract.method);
    }
  });
  ...
  it("resolves every contract to a well-formed path", () => {
    for (const contract of contracts) {
      const placeholders = Array.from(
        { length: contract.resolve.length },
        (_, i) => i,
      );
      const path = contract.resolve(...placeholders);
      expect(
        typeof path,
        `contract "${contract.id}" resolve should return a string`,
      ).toBe("string");
      expect(
        path,
        `contract "${contract.id}" path "${path}" should start with "/"`,
      ).toMatch(/^\//);
      expect(
        path,
        `contract "${contract.id}" path should not contain "//"`,
      ).not.toContain("//");
    }
  });
  ...
  it("resolves every contract path from its matching ENDPOINTS entry (no drift)", () => {
    for (const contract of contracts) {
      ...
      const placeholders = Array.from(
        {
          length: Math.max(
            contract.resolve.length,
            typeof endpointLeaf === "function"
              ? (endpointLeaf as () => unknown).length
              : 0,
          ),
        },
        (_, i) => i,
      ...
      expect(
        resolved,
        `contract "${contract.id}" drifts from ENDPOINTS entry`,
      ).toBe(expected);
    }
  });
  ...
  describe("resolveContractPath helper", () => {
    it("returns the same path as calling resolve directly", () => {
      const looseResolve = resolveContractPath as (
        c: EndpointContract,
        ...args: unknown[]
      ) => string;
      for (const contract of contracts) {
        ...
        const viaHelper = looseResolve(
          contract as EndpointContract,
          ...placeholders,
        );
  ...
  describe("admin additions", () => {
    ...
    it("binds admin.scim.test and admin.ssf.history to the ENDPOINTS paths", () => {
      expect(API_CONTRACTS.admin.scim.test.resolve()).toBe(
        ENDPOINTS.admin.scim.test,
      );
      expect(API_CONTRACTS.admin.ssf.history.resolve()).toBe(
        ENDPOINTS.admin.ssf.history,
      );
    });
  });
