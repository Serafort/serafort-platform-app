...
describe("API_ENDPOINTS registry", () => {
  ...
  it("keeps every string path as a well-formed absolute path", () => {
    ...
    for (const path of strings) {
      ...
      expect(
        path,
        `path "${path}" should not contain a double slash`,
      ).not.toContain("//");
      if (path !== "/") {
  ...
  it("keeps every dynamic builder producing a well-formed path", () => {
    const fns = collectLeafValues(API_ENDPOINTS).filter(
      (v): v is (...a: unknown[]) => unknown => typeof v === "function",
    );
  ...
  describe("admin registry additions", () => {
    it("exposes the adminMembers overrides group", () => {
      expect(ENDPOINTS.adminMembers.overrides(5)).toBe(
        "/api/admin/rbac/members/5/overrides",
      );
      expect(ENDPOINTS.adminMembers.addOverride(5)).toBe(
        "/api/admin/rbac/members/5/overrides",
      );
